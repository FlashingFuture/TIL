...
cannot_even = [[0] * N for _ in range(N)]
cannot_odd = [[0] * N for _ in range(N)]


def make_cannot_even(y, x, disable):     # y, x에 비숍을 놓거나 뺄 때 갱신해주는 함수
    # 좌하향 대각선
    temp_y, temp_x = y, x
    while temp_y < N and temp_x >= 0:
        cannot_even[temp_y][temp_x] = disable
        temp_y += 1
        temp_x -= 1
    # 우하향 대각선
...
def make_cannot_odd(y, x, disable):     # y, x에 비숍을 놓거나 뺄 때 갱신해주는 함수
    # 좌하향 대각선
    temp_y, temp_x = y, x
    while temp_y < N and temp_x >= 0:
        cannot_odd[temp_y][temp_x] = disable
        temp_y += 1
        temp_x -= 1
    # 우하향 대각선
    temp_y, temp_x = y + 1, x + 1
...
def backtrack_even(y, x, num_of_bishop):
    if y >= ((N - 1) // 2) * 2:
        if x >= ((N - 1) // 2) * 2:
            if plate[y][x] and not cannot_even[y][x]:




def backtrack_odd(y, x, num_of_bishop):
    if x >= (N // 2) * 2 - 2:
        if y == N:  # 마지막 점일 때
            ...
            if plate[y][x] and not cannot_odd[y][x]:
                make_cannot_odd(y, x, 1)
                backtrack_odd(y + 1, x % 2, num_of_bishop + 1)
                make_cannot_odd(y, x, 0)
            backtrack_odd(y + 1, x % 2, num_of_bishop)
    else:
        if plate[y][x] and not cannot_odd[y][x]:
            make_cannot_odd(y, x, 1)
